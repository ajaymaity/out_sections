...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                        ...
                        f"Patient Sex: {header.PatientSex}\n"
...
def get_series_for_inference(path):
    ...
    for each_dicom in dicoms:
        # TODO: Add a condition to check here
        if each_dicom.BodyPartExamined.lower() != 'brain':
            continue
        if not each_dicom.StudyDescription.lower().startswith('mr rcbv sequence'):
            continue
        series_for_inference.append(each_dicom)
...
if __name__ == "__main__":
    ...
    report_save_path = r"reports/report_" + str(header['SOPInstanceUID'].value) + str(time.time()) + ".dcm"
