...
def med_reshape(image, new_shape):
    ...
    shape_x, shape_y, shape_z = image_shape[0], image_shape[1], image_shape[2]
    if shape_x > new_shape[0]: shape_x = new_shape[0]
    if shape_y > new_shape[1]: shape_y = new_shape[1]
    if shape_z > new_shape[2]: shape_z = new_shape[2]
    reshaped_image[:shape_x, :shape_y, :shape_z] = image[:shape_x, :shape_y, :shape_z]
